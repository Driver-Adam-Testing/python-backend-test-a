...
class Backend(Construct):
    def __init__(self, scope: Construct, id: str, params: BackendParams):
        ...
        cluster_name = aws_ssm.StringParameter.value_from_lookup(scope,
                                                                 parameter_name="/baseline/infra/v2/ecs/cluster/name")
        cluster = aws_ecs.Cluster.from_cluster_attributes(self, id="BaselineCluster", cluster_name=cluster_name,
                                                          vpc=vpc)

        hosted_zone_id = aws_ssm.StringParameter.value_from_lookup(scope,
                                                                   parameter_name="/baseline/infra/v2/route53/hostedZoneId")
        hosted_zone_name = aws_ssm.StringParameter.value_from_lookup(scope,
                                                                     parameter_name="/baseline/infra/v2/route53/hostedZoneName")
        hosted_zone = aws_route53.HostedZone.from_hosted_zone_attributes(self, id="BaselineHostedZone",
                                                                         zone_name=hosted_zone_name,
                                                                         hosted_zone_id=hosted_zone_id)

        postgres_secret_name = aws_ssm.StringParameter.value_from_lookup(scope,
                                                                         parameter_name="/baseline/infra/v2/pythonBackend/postgresCredentialsName")
        postgres_secret = aws_secretsmanager.Secret.from_secret_name_v2(self, "PostgresSecret",
                                                                        secret_name=postgres_secret_name)

        modal_secret_name = aws_ssm.StringParameter.value_from_lookup(scope,
        ...
        auth0_secret_name = aws_ssm.StringParameter.value_from_lookup(scope,
        ...
        s3_secret_name = aws_ssm.StringParameter.value_from_lookup(scope,
                                                                   parameter_name="/baseline/infra/v2/pythonBackend/s3CredentialsName")
        s3_secret = aws_secretsmanager.Secret.from_secret_name_v2(self, "S3Secret", secret_name=s3_secret_name)

        github_secret_name = aws_ssm.StringParameter.value_from_lookup(scope,
                                                                       parameter_name="/baseline/infra/v2/pythonBackend/githubConfigurationsName")
        github_secret = aws_secretsmanager.Secret.from_secret_name_v2(self, "GitHubCredentials",
                                                                      secret_name=github_secret_name)

        openai_secret_name = aws_ssm.StringParameter.value_from_lookup(scope,
                                                                       parameter_name="/baseline/infra/v2/pythonBackend/openAIApiKeyName")
        openai_secret = aws_secretsmanager.Secret.from_secret_name_v2(self, "OpenAIApiKeyCredentials",
        ...
        container_secrets = {
            ...
            "AUTH0_MGMT_API_CLIENT_SECRET": aws_ecs.Secret.from_secrets_manager(auth0_secret,
        ...
        task_options = aws_ecs_patterns.ApplicationLoadBalancedTaskImageOptions(
            ...
            container_port=8000,
            log_driver=aws_ecs.LogDrivers.aws_logs(stream_prefix="python-backend",
        ...
        service.task_definition.task_role.attach_inline_policy(
            aws_iam.Policy(self, "CustomerSecretsRW", document=aws_iam.PolicyDocument(statements=[
                aws_iam.PolicyStatement(
                    effect=aws_iam.Effect.ALLOW,
                    actions=["secretsmanager:CreateSecret", "secretsmanager:ListSecrets",
                             "secretsmanager:DescribeSecret"],
                    resources=[
                        f"arn:aws:secretsmanager:{Stack.of(self).region}:{Stack.of(self).account}:secret:DRIVER_AI_CUSTOMER/*"]
                )
            ])))
        waf_visibility_config_ips = aws_wafv2.CfnWebACL.VisibilityConfigProperty(cloud_watch_metrics_enabled=True,
                                                                                 metric_name="MetricForWebACLCDK-IPs",
                                                                                 sampled_requests_enabled=True)
        waf_visibility_config = aws_wafv2.CfnWebACL.VisibilityConfigProperty(cloud_watch_metrics_enabled=True,
                                                                             metric_name="MetricForWebACLCDK",
                                                                             sampled_requests_enabled=True)
        waf_visibility_config_crs = aws_wafv2.CfnWebACL.VisibilityConfigProperty(cloud_watch_metrics_enabled=True,
                                                                                 metric_name="MetricForWebACLCDK-CRS",
                                                                                 sampled_requests_enabled=True)
        waf_rule_overrides = [
            aws_wafv2.CfnWebACL.RuleActionOverrideProperty(name="SizeRestrictions_BODY",
                                                           action_to_use=aws_wafv2.CfnWebACL.RuleActionProperty(
                                                               allow={})),
            aws_wafv2.CfnWebACL.RuleActionOverrideProperty(name="SizeRestrictions_URIPATH",
                                                           action_to_use=aws_wafv2.CfnWebACL.RuleActionProperty(
                                                               allow={})),
            aws_wafv2.CfnWebACL.RuleActionOverrideProperty(name="SizeRestrictions_QUERYSTRING",
                                                           action_to_use=aws_wafv2.CfnWebACL.RuleActionProperty(
                                                               allow={})),
            aws_wafv2.CfnWebACL.RuleActionOverrideProperty(name="GenericLFI_BODY",
                                                           action_to_use=aws_wafv2.CfnWebACL.RuleActionProperty(
                                                               allow={})),
            aws_wafv2.CfnWebACL.RuleActionOverrideProperty(name="GenericRFI_BODY",
                                                           action_to_use=aws_wafv2.CfnWebACL.RuleActionProperty(
                                                               allow={})),
        ]
        waf_rule_statement = aws_wafv2.CfnWebACL.StatementProperty(
            managed_rule_group_statement=aws_wafv2.CfnWebACL.ManagedRuleGroupStatementProperty(
                name="AWSManagedRulesCommonRuleSet", vendor_name="AWS", rule_action_overrides=waf_rule_overrides))
        crs_rule = aws_wafv2.CfnWebACL.RuleProperty(name="CRSRule", priority=1, statement=waf_rule_statement,
        ...
        if len(params.allowed_ips) > 0:
            whitelist_ip_set = aws_wafv2.CfnIPSet(self, "WhitelistIPs", ip_address_version="IPV4", scope="REGIONAL",
            ...
            ipset_rule = aws_wafv2.CfnWebACL.RuleProperty(name="AllowedIPs", priority=0,
        ...
        waf = aws_wafv2.CfnWebACL(self, "PythonBackendWAF", scope='REGIONAL',
                                  default_action=aws_wafv2.CfnWebACL.DefaultActionProperty(allow={}),
                                  visibility_config=waf_visibility_config, rules=waf_rules)
        waf_association = aws_wafv2.CfnWebACLAssociation(self, 'WebACLALBAssociation',
