...
class TeamRepository(BaseRepository[Team]):
    ...
    def count_teams(self, organization_id: str) -> int:
        ...
        Returns:
            Count of teams
        """
        query = select(func.count()).select_from(Team).where(Team.organization_id == organization_id)
        return self.session.exec(query).one()

    def count_teams_by_search(self, organization_id: str, search_query: str) -> int:
        """
