...
class TestTreeWalkFiltering:
    ...
    def repo_with_mixed_files(self, tmp_path):
        ...
        All non-binary, non-blacklisted text files are counted (matching
        the onboarding flow behavior). This includes:
        - Code files
        - Documentation (.md, .rst)
        - Config files (.json, .yaml, .toml)
        - Other text files (.gitignore)

        NOT counted:
        - Binary files
        - Blacklisted extensions (.exe, .dll, etc.)
    ...
    def test_all_text_files_counted(self, repo_with_mixed_files):
        """Verify all non-binary, non-blacklisted text files are counted.

        This matches the onboarding flow behavior where files don't need a
        recognized language in languages.yml to be counted as analyzable.
        """
        ...
        # All non-binary, non-blacklisted text files are counted:
        # main.py: 2 lines
        # utils.js: 3 lines
        # src/app.py: 2 lines
        # Dockerfile: 2 lines
        ...
        # .gitignore: 2 lines (NOW counted - matches onboarding flow)
        # Total: 24 lines
