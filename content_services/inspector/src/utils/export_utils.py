...
# TODO: anchor tags? 
def replace_driver_compatible_links_with_markdown_links(text: str, source_path: str) -> str:
    """
    ...
    for link in extracted_link:
        ...
        file_path = Path(*Path(link_part).parts[1:]) # Strips the first part of the path
        converted_file_path = (
            file_path.with_suffix("")
            .with_stem(file_path.stem + file_path.suffix.replace(".", "_"))
            .with_suffix(".md")
        )
        print(source_path, converted_file_path)
        ...
        if anchor_tag:
            ...
            new_url += "#" + anchor_name + "-" # added dash at end because of the driver links causing an extra space
