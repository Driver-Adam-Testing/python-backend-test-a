...
class AzureDevOpsAPIResources:
    ...
    def validate_token(self, token: str) -> dict[str, Any]:
        ...
        try:
            ...
            url = f"https://dev.azure.com/driverai/_apis/projects?api-version={self.api_version}&$top=1"
            ...
            with httpx.Client(follow_redirects=True) as client:
                ...
                if response.status_code in [200, 203]:
                    ...
                    if redirect_url:
                        ...
                        redirect_response = client.get(redirect_url, headers=headers, timeout=30.0)
                        if redirect_response.status_code == 200:
                            return {"status": "success", "data": redirect_response.json()}
                        else:
                            return {
                                "status": "error",
                        ...
                        return {"status": "error", "error": "Received redirect but no location header"}
                elif response.status_code == 401:
                    return {"status": "error", "error": "Invalid Personal Access Token"}
                elif response.status_code == 403:
                    return {"status": "error", "error": "Personal Access Token lacks required permissions"}
                else:
                    return {
                        "status": "error",
    ...
    def fetch_projects(self, token: str, organization: str) -> list[dict[str, Any]]:
        ...
        try:
            ...
            logger.error(f"Failed to fetch projects for organization {organization}: {e}")
            return []
        ...
        try:
            ...
            with httpx.Client() as client:
                ...
                for repo in repositories:
                    enriched_repo = self._enrich_repository_data(
                        token, project, repo
                    )
    ...
    def _enrich_repository_data(
        ...
        try:
                ...
                if response.status_code == 200:
                    ...
                    for ref in refs:
                        if ref.get("name", "").startswith("refs/heads/"):
                            ...
                            if branch_name == repo.get("defaultBranch", "").replace("refs/heads/", ""):
            ...
            logger.error(f"Failed to enrich repository data for {repo.get('name', 'unknown')}: {e}")
            return repo

    def download_repository_archive(
        self, token: str, organization: str, project: str, repository_id: str, ref: str = "main"
    ) -> bytes | None:
        ...
        try:
            ...
            with httpx.Client() as client:
                response = client.get(url, headers=headers, timeout=300.0)  # 5 minute timeout for large repos
                response.raise_for_status()
    ...
    def get_repository_content(
        self, token: str, organization: str, project: str, repository_id: str, path: str = "/", ref: str = "main"
    ) -> dict[str, Any] | None:
