...
# Purpose
The provided Python code defines an abstract base class `LlmClient`, which serves as a framework for interacting with various language model (LLM) APIs. This class is designed to be extended by concrete implementations that handle specific API kinds, such as OpenAI or Claude. The `LlmClient` class includes methods for initializing configurations, managing message histories, and executing tool requests asynchronously. It supports both single-shot and multi-shot interactions with LLMs, allowing for synchronous and asynchronous streaming of responses. The class also includes a mechanism for dynamically adding preset configurations for different LLM models, which can be initialized and used to create specific client instances.

The code is structured to provide a broad functionality for LLM interactions, encapsulating the complexity of handling different API kinds and response types. It defines a public API through its methods, which are intended to be used by other components or applications that require LLM capabilities. The use of abstract methods and class variables indicates that this file is part of a larger library, intended to be extended and customized for specific use cases. The `LlmClient` class is central to this functionality, providing a common interface and shared logic for various LLM client implementations.
...
- **Use**: This variable is used to control the export of the `LlmClient` class when the module is imported with a wildcard.
...
- **Description**: The `LlmClient` class is an abstract base class designed to manage and execute interactions with various language model configurations. It provides a framework for initializing and managing different language model presets, executing tools asynchronously, and generating responses either synchronously or as streams. The class supports both single-shot and multi-shot interactions, allowing for iterative message exchanges with language models. It also includes mechanisms for handling different API kinds and managing message histories, making it a versatile component for building language model clients.
- **Methods**:
...
The `__init__` method initializes an instance of the `LlmClient` class with a given configuration.
- **Inputs**:
    - `config`: An instance of `LlmConfig`, which provides configuration settings for the LlmClient; defaults to `LlmConfig.default()` if not provided.
- **Control Flow**:
    - The method assigns the provided `config` to the instance variable `self.config`.
- **Output**: This method does not return any value; it initializes the instance with the provided configuration.
- **Functions called**:
...
The `initialize_presets` class method dynamically adds preset methods to the `LlmClient` class based on predefined configurations.
- **Decorators**: `@classmethod`
- **Inputs**: None
- **Control Flow**:
    - Defines a nested function `_make` that takes a name and a factory function, returning a class method `_preset` that creates an `LlmClient` from a configuration.
    - Iterates over the `_PRESETS` dictionary of the class, which contains names and factory functions for different configurations.
    - For each entry in `_PRESETS`, it uses `setattr` to add a new class method to the class with the name from the dictionary and the method created by `_make`.
- **Output**: The method does not return any value; it modifies the class by adding new methods.
- **Functions called**:
    - [`python-backend/packages/shared/shared/v3/llms/clients/llm_client.LlmClient.from_config`](#LlmClientfrom_config)
- **See also**: [`python-backend/packages/shared/shared/v3/llms/clients/llm_client.LlmClient`](#LlmClient)  (Base Class)
...
The `from_config` class method returns a specific LlmClient subclass instance based on the provided LlmConfig's api_kind.
- **Decorators**: `@classmethod`
- **Inputs**:
    - `config`: An instance of LlmConfig that specifies the configuration for the LlmClient, including the api_kind which determines the specific client subclass to instantiate.
- **Control Flow**:
    - The method uses a match-case statement to determine the api_kind from the config parameter.
    - For each case corresponding to a specific ApiKind, it imports the appropriate client class and returns an instance of that class initialized with the provided config.
    - If the api_kind does not match any of the predefined cases, a ValueError is raised indicating an unsupported api_kind for the given provider.
...
- **Control Flow**:
    ...
    - If a `prompt` is provided, add it as a user message to the `history`.
    - Return the `history` object.
- **Output**: Returns an [`LlmMessageHistory`](../../interfaces/llm_message_history.py.md#LlmMessageHistory) object, which is either the provided `message_history` or a new one, potentially with an added user message if a prompt was given.
- **Functions called**:
...
- **Control Flow**:
    ...
    - Iterate over each tool request in `tool_requests`.
    - For each request, extract the parsed tool and append it to `called_tools`.
    ...
    - Create an asynchronous task for executing the tool and add it to `tasks`.
    - Use `asyncio.as_completed` to iterate over completed tasks.
    - For each completed task, await its result to get the tool message.
    - Add the tool message to the `history`.
    - Find the corresponding tool in `called_tools` using the tool response ID.
    ...
    - Handle exceptions by creating an error message, adding it to `history`, and yielding an [`ErrorStreamResponse`](../../interfaces/llm_stream_response.py.md#ErrorStreamResponse).
...
The `_generate` method is an abstract method intended to add a provider's response to a given message history synchronously.
- **Decorators**: `@abstractmethod`
- **Inputs**:
...
- **Control Flow**:
    ...
    - The method is expected to modify the `message_history` by adding the provider's response to it.
...
The `_generate_stream` method asynchronously generates a stream of messages by invoking a synchronous message generation method and yielding the last message from the message history.
- **Decorators**: `@abstractmethod`
- **Inputs**:
    ...
    - `tool_types`: An optional list of types of `LlmTool` that may be used in the message generation process.
- **Control Flow**:
...
- **Inputs**:
    - `prompt`: An optional string input representing the user's prompt to the language model.
...
- **Control Flow**:
    ...
    - Call the [`_generate`](llm_client_claude.py.md#ClaudeClient_generate) method to produce a response from the language model, updating the message history with the response.
    - Return the last message from the updated message history.
- **Output**: Returns the last `LlmMessage` from the message history, which contains the response from the language model.
- **Functions called**:
    - [`python-backend/packages/shared/shared/v3/llms/clients/llm_client.LlmClient._init_history`](#LlmClient_init_history)
    - [`python-backend/packages/shared/shared/v3/llms/clients/llm_client_claude.ClaudeClient._generate`](llm_client_claude.py.md#ClaudeClient_generate)
    - [`python-backend/packages/shared/shared/v3/interfaces/llm_message_history.LlmMessageHistory.last`](../../interfaces/llm_message_history.py.md#LlmMessageHistorylast)
- **See also**: [`python-backend/packages/shared/shared/v3/llms/clients/llm_client.LlmClient`](#LlmClient)  (Base Class)
...
- **Control Flow**:
    ...
    - Asynchronously iterate over the chunks generated by [`_generate_stream`](#LlmClient_generate_stream) with the initialized message history and specified response type.
    - For each chunk, check if it is an instance of `LlmMessage`.
...
The `multi_shot` method executes multiple iterations of generating responses and handling tool requests, returning the final message and a list of called tools.
- **Inputs**:
    - `prompt`: An optional string input that serves as the initial user message in the message history.
    ...
    - `tool_types`: An optional list of LlmTool types that specifies the types of tools available for use during the iterations.
    - `message_history`: An optional LlmMessageHistory object that represents the history of messages exchanged.
    - `datasource`: An optional DataSource object that provides data for tool execution.
- **Control Flow**:
    - Initialize an empty list called `called_tools` to keep track of tools that are called during the process.
    - Initialize the message history using [`_init_history`](#LlmClient_init_history) with the provided `message_history` and `prompt`.
    ...
    - Retrieve the last message from the message history and check if it is a `TOOL_CALL_REQUEST`.
    - If it is a `TOOL_CALL_REQUEST`, iterate over the tool requests, execute each tool, and add the execution result to the message history.
    - If the last message is not a `TOOL_CALL_REQUEST`, break out of the iteration loop.
    - Return the last message from the message history and the list of called tools.
- **Output**: A tuple containing the last `LlmMessage` from the message history and a list of `LlmTool` objects that were called during the iterations.
- **Functions called**:
    - [`python-backend/packages/shared/shared/v3/llms/clients/llm_client.LlmClient._init_history`](#LlmClient_init_history)
    - [`python-backend/packages/shared/shared/v3/interfaces/llm_message_history.LlmMessageHistory.add_message`](../../interfaces/llm_message_history.py.md#LlmMessageHistoryadd_message)
...
The `multi_shot_stream` method asynchronously generates and streams responses from a language model over multiple iterations, handling tool requests and yielding various response types.
- **Decorators**: `@abstractmethod`
- **Inputs**:
    - `prompt`: An optional string that serves as the initial input or question for the language model.
    - `iterations`: An integer specifying the number of iterations to perform, defaulting to 2.
    - `response_type`: An optional type specifying the expected response type from the language model.
    - `tool_types`: An optional list of tool types that can be used during the iterations.
    - `message_history`: An optional `LlmMessageHistory` object to maintain the conversation history.
    - `datasource`: An optional `DataSource` object used for tool execution.
- **Control Flow**:
    - Initialize an empty list `called_tools` to keep track of tools used during the iterations.
    - Initialize the message history using [`_init_history`](#LlmClient_init_history) with the provided `message_history` and `prompt`.
    ...
    - Set a flag `should_continue` to `True` to control the iteration loop.
    ...
    - Call [`_generate_stream`](#LlmClient_generate_stream) to asynchronously generate response chunks for the current iteration.
    - For each chunk, check its type and yield appropriate responses: [`ResponseChunkStreamResponse`](../../interfaces/llm_stream_response.py.md#ResponseChunkStreamResponse) for string chunks, [`ReferenceStreamResponse`](../../interfaces/llm_stream_response.py.md#ReferenceStreamResponse) and [`ResponseFullStreamResponse`](../../interfaces/llm_stream_response.py.md#ResponseFullStreamResponse) for assistant messages, and [`ErrorStreamResponse`](../../interfaces/llm_stream_response.py.md#ErrorStreamResponse) for unexpected types.
    - If a `LlmMessage` of kind `TOOL_CALL_REQUEST` is encountered, execute the requested tools asynchronously using [`_execute_tools_async`](#LlmClient_execute_tools_async) and yield their statuses.
    - If a `LlmMessage` of kind `ASSISTANT` is encountered, yield reference and full response streams, pause briefly, and set `should_continue` to `False` to stop further iterations.
    - Raise a `ValueError` if an unexpected chunk type is encountered.
