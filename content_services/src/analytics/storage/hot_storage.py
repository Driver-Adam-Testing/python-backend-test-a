...
class HotStorage:
    ...
    def upsert_repository_metrics(self, metrics: dict) -> None:
        ...
        """
        logger.debug(
            f"Upserting repository metrics for codebase {metrics.get('codebase_id')}"
        )
        ...
        update_clause = ", ".join(
            [f"{col} = EXCLUDED.{col}" for col in columns if col != "codebase_id"]
        )
        ...
        try:
            ...
            logger.info(
                f"Successfully upserted repository metrics for codebase {metrics.get('codebase_id')}"
            )
        except Exception as e:
    ...
    def get_repository_metrics(self, codebase_id: str) -> dict | None:
        ...
        result = self.conn.execute(
            "SELECT * FROM repository_metrics WHERE codebase_id = ?", [codebase_id]
        ).fetchone()
    ...
    def insert_daily_metrics(self, codebase_id: str, metrics: list[dict]) -> None:
        ...
        logger.info(
            f"Inserting {len(metrics)} daily metrics for codebase {codebase_id}"
        )
        ...
        try:
            ...
            logger.info(
                f"Successfully inserted daily metrics for codebase {codebase_id}"
            )
        except Exception as e:
    ...
    def get_daily_metrics(
        ...
        logger.debug(
            f"Fetching daily metrics for codebase {codebase_id} (range: {start_date} to {end_date})"
        )
    ...
    def insert_monthly_metrics(self, codebase_id: str, metrics: list[dict]) -> None:
        ...
        logger.info(
            f"Inserting {len(metrics)} monthly metrics for codebase {codebase_id}"
        )
        ...
        try:
            ...
            logger.info(
                f"Successfully inserted monthly metrics for codebase {codebase_id}"
            )
        except Exception as e:
    ...
    def upsert_branch_metrics(self, metrics: dict) -> None:
        ...
        """
        logger.debug(
            f"Upserting branch metrics for branch {metrics.get('branch_name')} in codebase {metrics.get('codebase_id')}"
        )
        ...
        update_clause = ", ".join(
            [
                f"{col} = EXCLUDED.{col}"
                for col in columns
                if col not in ("codebase_id", "branch_name")
            ]
        )
        ...
        try:
            self.conn.execute(sql, list(metrics.values()))
            logger.info(
                f"Successfully upserted branch metrics for branch {metrics.get('branch_name')}"
            )
        except Exception as e:
    ...
    def get_branch_metrics(self, codebase_id: str, branch_name: str) -> dict | None:
        ...
        """
        logger.debug(
            f"Fetching branch metrics for {branch_name} in codebase {codebase_id}"
        )
    ...
    def delete_branch_metrics(self, codebase_id: str, branch_name: str) -> None:
        ...
        """
        logger.debug(
            f"Deleting metrics for branch {branch_name} in codebase {codebase_id}"
        )
