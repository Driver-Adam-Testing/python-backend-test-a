...
def function_with_params(a: int, b: str, c: float | None = None):
    return f"{a} {b} {c}"


# 3) Function with various parameter types
def complex_params(positional_only, /, normal_param, *args, keyword_only, **kwargs):
...
def default_args_function(
    items: list[int] = None, mapping: dict = None, callback: Callable = lambda x: x
):
    if items is None:
        items = []
    if mapping is None:
...
def union_function(value: int | str) -> str:
    return str(value)
...
def complex_annotations(
    func: Callable[[int, str], bool], data: list[dict | None]
) -> Callable[[], None] | None:
    if not data:
        return None
