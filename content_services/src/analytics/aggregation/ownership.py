    ...
    for file_path, author_lines in file_author_lines.items():
        ...
        if total_abs_lines == 0:
            ...
            owners = [
                ...
                for email in author_lines.keys()
            ]
        else:
            owners = []
...
def get_top_owners(ownership: dict[str, list[dict]], top_n: int = 5) -> list[dict]:
    """Get top N owners across all files.
    
    Args:
        ownership: Result from calculate_ownership()
        top_n: Number of top owners to return
        
    Returns:
        List of top owners with total ownership stats
    """
    author_totals: dict[str, dict] = defaultdict(lambda: {
        'total_files': 0,
        'total_net_lines': 0,
    })
    ...
    for file_path, owners in ownership.items():
        for owner in owners:
            ...
            author_totals[email]['total_files'] += 1
            author_totals[email]['total_net_lines'] += owner['net_lines']
    ...
    top_owners = [
        {
            ...
            'total_net_lines': stats['total_net_lines'],
        }
        for email, stats in author_totals.items()
    ]
    
    top_owners.sort(key=lambda x: x['total_net_lines'], reverse=True)
    return top_owners[:top_n]
