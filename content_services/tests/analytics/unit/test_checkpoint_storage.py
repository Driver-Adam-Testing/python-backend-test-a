...
class TestCheckpointLocalCache:
    """Tests for local file checkpoint cache."""

    def test_write_local_checkpoint(self, tmp_path):
        """Should write checkpoint to local file."""
        from analytics.checkpoint import ExtractionCheckpoint, write_local_checkpoint
        ...
        write_local_checkpoint(checkpoint, local_path)

        assert local_path.exists()

        # Verify it's valid JSON
        data = json.loads(local_path.read_text())
        assert data["codebase_id"] == "test-uuid"

    def test_read_local_checkpoint(self, tmp_path):
        """Should read checkpoint from local file."""
        from analytics.checkpoint import ExtractionCheckpoint, read_local_checkpoint
        ...
        local_path = tmp_path / "checkpoint.json"
        local_path.write_text(checkpoint.model_dump_json())

        result = read_local_checkpoint(local_path)

        assert result is not None
        assert result.codebase_id == "test-uuid"

    def test_read_missing_local_returns_none(self, tmp_path):
        """Should return None for missing local file."""
        from analytics.checkpoint import read_local_checkpoint
    ...
    def test_read_corrupted_local_returns_none(self, tmp_path):
        """Should return None for corrupted local file."""
        from analytics.checkpoint import read_local_checkpoint

        local_path = tmp_path / "corrupted.json"
