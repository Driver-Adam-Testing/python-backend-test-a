...
def process_auth0_event_test(event_dict: dict[str, Any]) -> Auth0EventProcessingResult:
    ...
    with (
        patch("app.iam.auth0_event_processor.engine") as mock_engine,
        patch("app.iam.auth0_event_processor.Session", mock_session_factory),
        patch("app.iam.auth0_event_processor.User", MockUserModel),
        patch("app.iam.auth0_event_processor.Organization", MockOrganizationModel),
        patch("app.iam.auth0_event_processor.OrgMembership", MockOrgMembershipModel),
        patch("app.iam.auth0_event_processor.select") as mock_select,
    ):
        # Mock the select function to return a query object that our MockSession can handle
        mock_select.return_value = MagicMock()

        # Import and call the real processor
        from app.iam.auth0_event_processor import process_auth0_event

        return process_auth0_event(event_dict)
